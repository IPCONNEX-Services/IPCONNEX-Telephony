...
class TelephonyPartnerContract(Document):
    def validate(self):
        if not self.customer and not self.supplier:
            frappe.throw("Set at least one of Customer or Supplier.")
...
def get_partner_balance(partner):
    ...
    doc = frappe.get_cached_doc("Telephony Partner Contract", partner)
